...
router.get('/', async (req, res, next) => {
  try {
    ...
    const list = await Localizacao.find(page, size, sort, direction, filter)
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const serializer = new Serializer(res.getHeader('Content-Type'))
    res.status(200).send(serializer.serialize(localizacao))
  } catch (error) {
    next(error)
  }
})
...
router.post('/', async (req, res, next) => {
  ...
  try {
    ...
    const localizacao = new Localizacao(req.body)
    const result = await localizacao.add()
